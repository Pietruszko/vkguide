...
void VulkanEngine::init_vulkan() {
  ...
  // use vkbootsrap to get a Graphics queue
...
void VulkanEngine::init_sync_structures() {
  ...
  // we want the fenceto start signalled so we can wait on it on the first frame
...
void VulkanEngine::draw() {
  ...
  // we will use this command buffer exactly once, so we want to let vulkan know
  // that
  ...
  // this will flash with a 120 frame per period
  ...
  // finalize the command buffer (we can no longer add commands, but it can be
  // executed)
  VK_CHECK(vkEndCommandBuffer(cmd));

  // prepare the submission to the queue
  // we want to wait on the _presentSemaphore, as that semaphore is signaled
  // when swapchain is ready we will signal the _renderSemaphore, to signal that
